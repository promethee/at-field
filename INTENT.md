...
- *The theme's own words and word family are excluded.* A hit on the theme's own vocabulary would inflate saturation with evidence the theme already implies. This is circular, the same objection as subject-to-topic promotion. The exact theme words are always dropped, and words sharing a stem with them are dropped too ("economic" and "economist" for "economy"). Before this rule the field kept "religionist" for the theme "religion", which wastes a slot, and a word like "economic" would have added hits that restate the theme.
- *Variants of an already-kept word are dropped, and the first one listed wins.* A word sharing a stem (by default its first 5 letters) with a kept word is treated as an inflection or derivation of it. Real French output padded a 31-word field with 13 variants of 6 roots (lecture/lectrices, voyage/voyageur/voyageuse, photographie/photographe/photographique), which inflates the field size and deflates saturation. The rule is deliberately conservative. Five letters avoids merging distinct short roots (jeu/jeune, parc/parce, art/article), at the cost of missing variants of short roots (art/artistes, lecture/lecteur) and one known false merge (marché/marchandise). On the real field it dropped 8 words (31 to 23, 6% to 8.7%). The stem length is adjustable with `--stem-length` (3 to 12, or 0 to turn stem filtering off), and both stem rules use it. Themes shorter than the stem length keep only the exact rule, since a shorter stem would also catch unrelated words ("war" and "warm"). The rule can drop a useful word that shares a stem with the theme ("nationalism" for "nation"), so the flag is the escape hatch, and the report says when the default was changed. `--lexic` skips the filter.

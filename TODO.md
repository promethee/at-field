...
Steps 1-3 of the pivot are done (input parser, `cli.ts` rewire, obsolete
module/test removal) and verified with real end-to-end runs. `--preset`,
`--whisper-model`, `--max-duration`, `--start`, `--end`, and all
audio/Whisper machinery no longer exist. Remaining: INTENT.md rewrite,
README rewrite, tarball retest (Steps 4-6, see the pivot section).

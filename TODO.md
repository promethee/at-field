...
Steps 1-4 of the pivot are done (input parser, `cli.ts` rewire, obsolete
module/test removal, full INTENT.md rewrite) and verified with real
end-to-end runs. `--preset`, `--whisper-model`, `--max-duration`,
`--start`, `--end`, and all audio/Whisper machinery no longer exist.
Remaining: README rewrite, tarball retest (Steps 5-6, see the pivot
section).
